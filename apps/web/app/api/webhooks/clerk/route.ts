...
export async function POST(req: Request) {
  ...
  switch (type) {
    ...
    case 'user.updated':
      try {
        const username = user.external_accounts?.[0]?.username || user.username || user.id;
        ...
        if (error) {
          return NextResponse.json({ error: 'Failed to sync user with Supabase', details: error }, { status: 500 });
        }
      } catch (error) {
        return NextResponse.json({ error: 'Unexpected error during user sync', details: error }, { status: 500 });
      }
      break;

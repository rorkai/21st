...
BEGIN
    ...
    LOOP
        ...
        IF dep_component_id IS NOT NULL THEN
            -- Insert direct dependency (depth 1)
            INSERT INTO component_dependencies_closure 
                (component_id, dependency_component_id, depth)
            VALUES 
                (p_component_id, dep_component_id, 1)
            ON CONFLICT DO NOTHING;
...
CREATE OR REPLACE FUNCTION "public"."update_component_with_tags"("p_component_id" integer, "p_name" "text", "p_description" "text", "p_license" "text", "p_preview_url" "text", "p_tags" "jsonb") RETURNS "void"
    LANGUAGE "plpgsql"
    AS $$BEGIN
...
CREATE TABLE IF NOT EXISTS "public"."components" (
    ...
    "demo_component_names" "jsonb" NOT NULL,
    "direct_registry_dependencies" "jsonb" DEFAULT '[]'::"jsonb" NOT NULL,
    "registry" "text" DEFAULT 'ui'::"text",
    "fts" "tsvector"
);
...
CREATE OR REPLACE VIEW "public"."components_with_username" AS
 SELECT "components"."id",
    ...
    "components"."license",
    "components"."fts",
    "components"."demo_component_names",
...
CREATE POLICY "Allow everything to service role and supabase_admin" ON "public"."component_dependencies_closure" TO "dashboard_user", "service_role", "supabase_admin", "postgres", "supabase_auth_admin", "supabase_storage_admin", "supabase_replication_admin", "supabase_realtime_admin" USING (true) WITH CHECK (true);
